...
export default function BasicFace({
  ...
  useEffect(() => {
    ...
    if (ctx) renderBasicFace({ ctx, mouthScale, eyeScale, color, hatYOffset });
  }, [canvasRef, volume, eyeScale, mouthScale, color, scale, hatYOffset]);
  ...
  return (
    <canvas
      ...
      width={canvasSize}
      height={canvasSize}
      style={{ display: 'block', transform: `translateY(${hoverPosition}px) rotate(${tiltAngle}deg)` }}
    />
  );
}
