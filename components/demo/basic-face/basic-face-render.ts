...
export function renderBasicFace({ ctx, eyeScale, mouthScale, color, hatYOffset = -100 }: RenderFaceProps) {
  ...
  ctx.translate(-213.38, -217.61); // Центр SVG viewBox
  ...
  const hatPath = new Path2D(
    "M26.52,114.02c-.57-5,5.13-5.63,9.26-17.87,5.79-17.14-2.08-25.62-2-52.28.06-15.66.13-26.67,7.28-34.43C52,-.34,72.67,2.56,80.13,3.5c46.19,5.85,93.12-1,139.67,0,49.25,1.1,98.52,11.48,147.6,7.28,5.38-.46,19.59-1.83,27.14,6.62,8.42,9.42,1.68,23.33.66,51-1.2,32.58,7.28,37.37,3.32,48.33-8.71,24-81.7,38.25-188.65,36.4C32.28,148.02,27.71,124.49,26.52,114.02Z"
  );
  ...
  // Червоний хрест (вертикаль)
  ctx.fillStyle = '#e30613';
  ctx.beginPath();
  ctx.roundRect(198.44, 50.52, 27.94, 50.99, 7.86);
  ctx.fill();
  
  // Червоний хрест (горизонталь)
  ctx.beginPath();
