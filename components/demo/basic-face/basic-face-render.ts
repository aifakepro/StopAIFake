...
// Вспомогательная функция для глаз
const eye = (
  ctx: CanvasRenderingContext2D,
  pos: [number, number],
  radius: number,
...
export function renderBasicFace(props: BasicFaceProps) {
  ...
  // Масштабирование SVG под Canvas
  const scaleX = width / 426.76;
  const scaleY = height / 430.99;
  ...
  // Круг
  ctx.beginPath();
  ctx.arc(213.38, 217.61, 212.99, 0, 2 * Math.PI);
  ctx.fillStyle = "#fee9cb"; // базовый fill
  ctx.fill();
  ...
  ctx.fillRect(198.44, 50.52, 27.94, 50.99); // вертикальная
  ctx.fillRect(181.83, 62.98, 61.17, 27.94); // горизонтальная
